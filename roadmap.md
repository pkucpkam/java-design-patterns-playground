...
- [ ] **Factory Method** - Định nghĩa một interface để tạo đối tượng, nhưng để các subclass quyết định class nào sẽ được khởi tạo.
- [ ] **Abstract Factory** - Cung cấp một interface để tạo các họ đối tượng liên quan hoặc phụ thuộc lẫn nhau mà không chỉ định các class cụ thể.
- [ ] **Builder** - Tách biệt việc xây dựng một đối tượng phức tạp khỏi biểu diễn của nó để cùng một quá trình xây dựng có thể tạo ra các biểu diễn khác nhau.
- [ ] **Prototype** - Xác định các loại đối tượng cần tạo bằng cách sử dụng một instance mẫu (prototype), và tạo các đối tượng mới bằng cách sao chép (copy) mẫu này.
...
- [ ] **Adapter** - Cho phép các interface không tương thích có thể làm việc cùng nhau.
- [ ] **Bridge** - Tách biệt phần trừu tượng (abstraction) khỏi phần triển khai (implementation) để cả hai có thể thay đổi độc lập.
